...
void init_paremeters(int argc, char**argv, model& our_model)
{
	...
	our_model.method = 2;
	our_model.s_threshold = 80;
	our_model.t_threshold = 20;*/
...
void output_result(model& our_model)
{
	...
	cout << "The spatiotemporal network K-function value is: " << our_model.K_value << endl;
}
...
{
	int num_points = e.PS.size();
	e.t.num_points = num_points;
	e.t.Point_Matrix_distance = new double*[num_points];
	for (int i = 0; i < num_points; i++)
		e.t.Point_Matrix_distance[i] = new double[2];

	for (int i = 0; i < num_points; i++)
	{
		e.t.Point_Matrix_distance[i][0] = e.PS[i].time;
		e.t.Point_Matrix_distance[i][1] = e.PS[i].dist_n1;
	}
