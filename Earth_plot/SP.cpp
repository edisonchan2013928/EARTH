...
void init_Dijkstra(model& our_model, PQ& pq)
{
	...
	if (our_model.method == 2 || our_model.method == 3 || our_model.method == 4 || our_model.method == 5) //SPS, EAR, MTS, and EARTH methods
	{
...
void Dijkstra(model& our_model, vector<sp_node>& sp_node_vec, vector<int>& access_edge_list, bool*is_scan_edge_list)
{
	...
	while (pq.size() > 0)
	{
		...
		//discard node with value more than the spatial threshold s
		if (sp_node_vec[SP_node.node_index].cur_sp_value > our_model.s_tau_vec[our_model.cur_s_tau_index])
			continue;
